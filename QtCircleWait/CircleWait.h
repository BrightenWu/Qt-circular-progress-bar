#ifndef CIRCLEWAIT_H
#define CIRCLEWAIT_H

#include <QWidget>

class QCircleWaitWidget : public QWidget
{
    Q_OBJECT
public:
    explicit QCircleWaitWidget(QWidget *parent = 0);

    void setColor(QColor color);
    void setClockwiseRotate(bool clockwise);
    void setRotateDelta(int delta);

	void setText(const QString &str);
	void setFontSize(int nSize);
	void setFont(const QString &str);

protected:
    void paintEvent(QPaintEvent *);
    void drawCircleWait(QPainter& p);  //!< 圆形等待

private:
    QColor m_color;
    bool m_clockwiseRotate;  //!< 顺时针旋转
    int m_rotateDelta;  //!< 旋转步进角度
    int m_rotateAngle;  //!< 旋转角度
	QString m_strText;  //!< 提示文本
	QString m_strFont = "Arial";  //!< 字体类型
	int m_nFontSize = 20;  //!< 字体大小

    QTimer* m_timer;
};

#endif // CIRCLEWAIT_H
